...
```bash
Sampleloggerjava$ sam build
Sampleloggerjava$ sam deploy --guided
```
...
```bash
Sampleloggerjava$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Sampleloggerjava$ sam local start-api
Sampleloggerjava$ curl http://localhost:3000/
```
...
```bash
Sampleloggerjava$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
Sampleloggerjava$ cd HelloWorldFunction
